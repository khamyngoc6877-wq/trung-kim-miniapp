...
function variantNameFromOrderItemName(name: string): string | undefined {
...
export async function updateOrderStatus(id:string,status:OrderStatus){
  ...
  try {
    ...
    const orderResult = await c.query(
      `select * from public.orders where id=$1 limit 1 for update`,
      [orderId],
    );
    ...
    if (status === "completed" && !current.stock_deducted_at) {
      ...
      for (const item of itemResult.rows) {
        ...
        if (variantName) {
          ...
          const pr = await c.query(
            `select id, stock from public.products where id=$1 limit 1 for update`,
            [productId],
          );
      ...
      await c.query(
        `update public.orders set stock_deducted_at=now() where id=$1`,
        [orderId],
      );
    }
