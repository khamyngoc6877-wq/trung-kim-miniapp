...
async function buildSafeMember(client: any, row: any) {
  const [historyResult, vouchersResult] = await Promise.all([
    client.query(
      `select id,type,points,description,order_id,created_at
       from public.point_history where member_id=$1 order by created_at desc limit 100`,
      [row.id],
    ),
    client.query(
      `select voucher_code,name,discount_amount,points_spent,created_at,end_at
       from public.member_vouchers where member_id=$1 order by created_at desc`,
      [row.id],
    ),
  ]);
  ...
  return {
    ...
    pointHistory: historyResult.rows.map((h: any) => ({
      id: h.id,
      type: h.type,
    ...
    vouchers: vouchersResult.rows.map((v: any) => ({
      code: v.voucher_code,
      name: v.name,
...
export async function registerMember(input: {
  name: string; phone: string; email?: string; address?: string; password: string;
}) {
  ...
  try {
    const { rows } = await pool.query(
      `insert into public.members
       ...
       values ($1,$2,$3,$4,$5,$6,0,$7,$8,$7,$7) returning *`,
      [id, input.name.trim(), phone, input.email?.trim() || null, input.address?.trim() || null,
       hash(input.password), now.toISOString(), expireAt.toISOString()],
    );
    return buildSafeMember(pool, rows[0]);
  } catch (e: any) {
    if (e?.code === "23505") throw new Error("Số điện thoại đã đăng ký");
    throw e;
  }
}
...
export async function loginMember(phone: string, password: string) {
  ...
  const { rows } = await pool.query(
    "select * from public.members where phone=$1 and password_hash=$2 limit 1",
    [normalizedPhone, hash(password)],
  );
...
export async function addOrderPoints(memberId: string, orderId: string, subtotal: number) {
  ...
  try {
    ...
    const rewarded = await client.query(
      `insert into public.rewarded_orders(member_id,order_id) values($1,$2)
    ...
    if (earnedPoints > 0) {
      ...
      await client.query(
        `insert into public.point_history(id,member_id,type,points,description,order_id,created_at)
         values($1,$2,'earn',$3,$4,$5,now())`,
        [crypto.randomUUID(), memberId, earnedPoints, `Cộng điểm đơn hàng ${orderId}`, orderId],
      );
    }
    await client.query("commit");
...
export async function redeemMemberPoints(memberId: string) {
  const client = await pool.connect();
  try {
    await client.query("begin");
    const { rows } = await client.query("select * from public.members where id=$1 for update", [String(memberId).trim()]);
    const member = rows[0];
    if (!member) throw new Error("Không tìm thấy tài khoản thành viên");
    ...
    await client.query(
      `insert into public.vouchers
       ...
       values($1,$2,$3,'fixed',$4,$4,1,0,$5,$6,true,$5,$5)`,
      [voucherId, code, "Voucher đổi từ 100 điểm", REDEEM_VALUE, now.toISOString(), endAt.toISOString()],
    );
    ...
    await client.query(
      ...
      [crypto.randomUUID(), member.id, code, "Voucher đổi từ 100 điểm", REDEEM_VALUE, REDEEM_POINTS, now.toISOString(), endAt.toISOString()],
    );
    await client.query(
      `insert into public.point_history(id,member_id,type,points,description,created_at)
       values($1,$2,'redeem',$3,$4,$5)`,
      [crypto.randomUUID(), member.id, -REDEEM_POINTS, "Đổi 100 điểm lấy voucher 20.000đ", now.toISOString()],
    );
