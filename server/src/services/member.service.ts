...
const filePath = path.resolve(
  process.env.MEMBER_STORE_FILE || "./data/members.json",
);

let queue: Promise<unknown> = Promise.resolve();

async function readDb(): Promise<MemberDb> {
  try {
...
async function writeDb(db: MemberDb): Promise<void> {
...
function mutate<T>(fn: (db: MemberDb) => T | Promise<T>): Promise<T> {
...
function ensureLegacyHistory(member: Member): void {
...
export async function loginMember(
  ...
  const member = Object.values(db).find(
    (item) =>
...
export async function addOrderPoints(
  memberId: string,
  orderId: string,
  subtotal: number,
) {
  return mutate((db) => {
    ...
    if (
      member.rewardedOrderIds.includes(orderId)
    ) {
      return {
        ...safe(member),
        earnedPoints: 0,
...
export async function redeemMemberPoints(
  ...
  const prepared = await mutate((db) => {
    ...
    if (member.points < REDEEM_POINTS) {
      throw new Error(
        `Bạn đang có ${member.points} điểm. Cần đủ ${REDEEM_POINTS} điểm để đổi voucher.`,
      );
    }
  ...
  try {
    ...
    await mutate((db) => {
      ...
      member.points += REDEEM_POINTS;
      member.vouchers = member.vouchers!.filter(
        (item) =>
          item.code !== prepared.voucher.code,
      );
      member.pointHistory =
        member.pointHistory!.filter(
          (item) =>
            item.voucherCode !==
    ...
    console.error(
      "Create redeemed voucher failed:",
      error,
    );

    throw new Error(
      "Không tạo được voucher. Điểm của bạn đã được hoàn lại, vui lòng thử lại.",
    );
  }
