...
export async function createVoucher(input) {
    ...
    try {
        const { rows } = await pool.query(`insert into public.vouchers
       ...
       returning *`, [crypto.randomUUID(), code, String(input.name).trim(), input.type, Number(input.value),
...
export async function updateVoucher(id, input) {
    validateInput(input);
    try {
        const { rows } = await pool.query(`update public.vouchers set
       code=$2,name=$3,type=$4,value=$5,min_order=$6,max_discount=$7,usage_limit=$8,
       start_at=$9,end_at=$10,active=$11,updated_at=now()
       where id=$1 returning *`, [String(id).trim(), normalizeCode(input.code), String(input.name).trim(), input.type,
...
export async function validateVoucher(codeValue, subtotalValue) {
    ...
    if (subtotal < voucher.minOrder)
        return { valid: false, message: `Đơn hàng tối thiểu ${voucher.minOrder.toLocaleString("vi-VN")}đ`, discountAmount: 0 };
...
export async function consumeVoucher(codeValue) {
    ...
    if (!code)
        return;
    await pool.query(`update public.vouchers set used_count=used_count+1, updated_at=now()
     where code=$1 and active=true and (usage_limit is null or used_count < usage_limit)`, [code]);
}
